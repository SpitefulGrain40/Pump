...
- **Measurement history** — `pump-measurement-history` log, written from three entry points (Measurements modal, Settings' Profile fields, Coach `[LOG_MEASUREMENT]`), all now correctly linked to the same history the dashboard/Coach read from (see Troubleshooting, fixed 2026-07-10/11). Powers body-fat %, waist, and lean-mass trends (lean mass = weight × (1 − bf%)). Navy is always the tracked body-fat trend; a manual/DEXA reading shows as a separate reference marker on the chart, never overriding Navy (trend-consistency by design).
...
- **Vitest** unit tests for cycle logic (`utils/schedule.test.js`), goal/metrics pure logic (`utils/goal.js`, `utils/metrics.js`), body-fat/lean-mass resolution (`utils/calculations.test.js`), Progress-tab maths (`utils/progressCalcs.test.js`), and smart food logging (`utils/foodLibrary.test.js`, `utils/cofid.test.js`, `utils/openFoodFacts.test.js`, `utils/nutritionResolver.test.js`, `utils/dataSchemas.test.js`) — 147 tests, `npm test`
...
Fix unified all "current value" reads onto the history arrays (the real source of truth), profile fields as fallback only: `getNavyBodyFat`/`resolveBodyFat` now take a measurement snapshot/history param (backward compatible — old call sites passing just `profile` still work). `Dashboard.jsx`, `context.js`'s `buildContextFromState` (now also takes `measurementHistory`), and Coach's system prompt all read from it instead of `profile.currentWeight`/inline-duplicated Navy math (the inline copy was removed in favour of importing `calculations.js`). Coach's `LOG_WEIGHT`/`LOG_MEASUREMENT` handlers now also call `updateProfile` for consistency with the modals.
Verified end-to-end live (not just unit tests): logging a measurement now correctly updates `profile.waistCircumference` despite the modal closing itself; Dashboard's own Body Fat/Lean Mass panel now matches GoalCard/SecondaryMetricStrip exactly; `buildCoachSystemPrompt`'s actual output string was checked directly against a deliberately-stale profile + fresh history to confirm it shows the fresh values.

**Follow-up, 2026-07-11 — two more gaps surfaced by real usage:**
1. **Settings tab edits weren't "linked"** — `Settings.jsx`'s weight/waist/neck/hip/bodyFatManual fields only ever called `updateProfile(...)`, never `logWeight`/`logMeasurement`. Once the fix above made Dashboard/GoalCard/Coach *prefer* the history arrays over the profile snapshot, a Settings-only edit got silently shadowed by whatever older history entry already existed — appearing to "do nothing." Fixed: Settings now also writes to history on every field edit (`syncWeight`/`syncMeasurement` in `Settings.jsx`), merging with the current profile's other fields so editing just one field doesn't null out an already-logged one for today (`createMeasurementEntry` defaults missing fields to `null`, not `undefined`, so a naive partial `logMeasurement` call would have wiped unrelated fields).
2. **`getNavyBodyFat`/`resolveBodyFat` signature changed from a single `latestMeasurement` snapshot to the full `measurementHistory` array** — searches for the most recent snapshot that actually has waist+neck together (matching how `utils/metrics.js` already searched), rather than assuming the single latest entry overall always has what's needed (it might be a manual-only reading with no waist/neck at all). All callers (`Dashboard.jsx`, `context.js`) updated to pass the array.
3. **Manual/DEXA body fat readings redesigned as a reference series, not a competing "freshest wins" source.** An interim design (manual overrides Navy when logged more recently) was built then deliberately reverted before shipping — Navy and DEXA use different methodology, so letting the *displayed* number silently swap between them would make an ordinary measurement-method change look like a real body-composition jump. Final design: **Navy is always the trend baseline** when computable (`resolveBodyFat`, unchanged priority from before this whole fix — manual is fallback-only, when Navy genuinely can't be computed). Manual/DEXA readings are exposed as their own series (`utils/metrics.js`'s `bodyfat.getManualSeries`) and plotted as a distinct marker (purple diamond, `showLine: false`) alongside the Navy trend line via a new `alignSeriesWithReference` helper (unions the two series' dates onto one label set, since a DEXA-only day might not coincide with a Navy-computable one) — in both `GoalCard.jsx` (primary metric) and `SecondaryMetricStrip.jsx` (expandable secondary chart). `bodyfatValueFor`/`bodyfatForDate` (used by `leanmass` too) were also made Navy-only for the same trend-consistency reason — a manual reading on a snapshot no longer silently swaps out lean mass's bf% input either. Verified live: entering a manual 22% (with Navy at 16.9%) left the main number at 16.9%, added a "manual: 22%" secondary annotation, and rendered as a separate purple marker on the chart, visually apart from the green Navy line — confirmed via screenshot, not just unit tests.
