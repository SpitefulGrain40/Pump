...
- **CoFID/library ranking has no confidence floor in the suggestions dropdown**: `searchSuggestions` (the live-typing dropdown) still always shows the top-scoring match, even a weak one, so you can see and ignore a bad guess. Fixed 2026-07-06 for the *committing* path only — `resolveNutrition` (barcode/photo/manual-submit) now requires `minCoverage: 0.5` (at least half the query's words must match) before trusting a library or CoFID hit, falling through to OFF/AI instead of silently mis-logging (e.g. "Vegan Protein 360 Protein Works BLACK" was matching a saved "Vegan Salted Caramel Ice Cream" library entry on the word "vegan" alone). See `fuzzyMatch`'s `minCoverage` param in `utils/foodLibrary.js`.
...
### Meal logging matches the wrong food (barcode, photo, or manual)
Check whether the personal food library (`pump-food-library`) has an unrelated saved item that shares just one generic word with the query (e.g. "Vegan Salted Caramel Ice Cream" matching any "vegan ..." query). `resolveNutrition`'s library/CoFID tiers require `minCoverage: 0.5` (fixed 2026-07-06) so a single shared word should no longer be trusted — if it still happens, the query and candidate share ≥50% of their words and it's a genuine ranking issue in `fuzzyMatch` (`utils/foodLibrary.js`), not the confidence-floor bug. The live suggestions dropdown (`searchSuggestions`) intentionally stays permissive (no floor) so weak candidates are still visible there for the user to reject — that's by design, not a bug.
